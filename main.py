...
app.index_string = app.index_string.replace("</head>", f"<style>{NEON_CSS}</style></head>")
...
def make_nav():
    return html.Div(className="header", children=[
        html.Div(className="brand", children=[html.Div(className="logo", children="S"), html.H1("S.E.N.N.A. — RAGE DASH")]),
...
def win_probability_model():
    # Load CSV data
    df = pd.read_csv("data/f1_history.csv")

    X = df[["wins_avg", "podiums_avg", "reliability", "aero_rating"]]
    ...
    # Predict again (2026 scenario simulation)
    predictions = model.predict(X_scaled)

    # Clamp values between 0 and 100
    predictions = predictions.clip(0, 100)

    return df["team"], predictions
...
def page_home():
    return html.Div(className="container", children=[
        html.Div(className="card", children=[html.H2("S.E.N.N.A. — Rage Edition"), html.P("High-performance F1 analytics with local assets.")]),
        html.H3("Teams", style={"marginTop":"18px"}),
        html.Div(className="team-grid", children=[team_card(t) for t in TEAMS]),
        html.Div(className="footer", children="S.E.N.N.A. — Tribute to Ayrton Senna")
    ])
        ...
        html.Div(className="team-page", children=[
            html.Div(className="team-main", children=[
                html.Div(className="card", children=[
                    ...
                    html.Div("Engine: " + t["engine_name"], style={"color":"var(--muted)"}),
                    html.Div(className="stat-row", children=[
                        html.Div(className="stat", children="Power: " + t["engine_stats"]["Combined Power"]),
                        html.Div(className="stat", children="ERS: " + t["engine_stats"]["ERS"]),
                        html.Div(className="stat", children="PU: " + t["engine_stats"]["Type"])
                    ]),
                    html.P(t.get("history",""), style={"color":"var(--muted)"})
                ]),
                html.H3("Drivers", style={"marginTop":"18px"}),
                html.Div(children=[
                    html.Div(className="driver-card", children=[
                        html.Img(src=d["photo"], className="driver-photo"),
                        html.Div(children=[html.Div(html.Strong(d["name"] + f"  #{d['no']}")), html.Div(style={"color":"var(--muted)"}, children=f"Wins: {d.get('wins',0)}  • Podiums: {d.get('podiums',0)}  • Starts: {d.get('starts',0)}")])
                    ]) for d in t["drivers"]
                ])
            ]),
            html.Div(className="team-side", children=[
                html.Div(className="card", children=[html.H4("Quick Stats"), html.Div("Season form: RAGE mode", style={"color":"var(--neon)"}), html.Ul(children=[html.Li("Downforce: HIGH"), html.Li("Top Speed: ELITE"), html.Li("Reliability: STRONG")], style={"color":"var(--muted)"})]),
                html.Div(className="card", style={"marginTop":"12px"}, children=[html.H4("Recent Upgrades"), html.Ul(children=[html.Li("Aero floor update"), html.Li("Cooling inlet patch")], style={"color":"var(--muted)"})])
            ])
        ])
    ])
...
def page_drivers():
    ...
    for t in TEAMS:
        for d in t["drivers"]:
            rows.append(html.Div(className="card", style={"marginBottom":"10px"}, children=[html.Div(style={"display":"flex","gap":"12px","alignItems":"center"}, children=[html.Img(src=d["photo"], style={"width":"80px","height":"80px","objectFit":"cover","borderRadius":"8px"}), html.Div(children=[html.Div(html.Strong(d["name"] + f"  —  {t['name']}")), html.Div(style={"color":"var(--muted)"}, children=f"Wins: {d.get('wins',0)} • Podiums: {d.get('podiums',0)} • Starts: {d.get('starts',0)}")])])]))
    return html.Div(className="container", children=[html.H2("Drivers", className="section-title")] + rows)

def page_champions():
    champs=[{"year":2023,"winner":"Max Verstappen"},{"year":2022,"winner":"Max Verstappen"},{"year":2021,"winner":"Max Verstappen"}]
...
def page_predictions():
    teams, probs = win_probability_model()

    fig = px.bar(
        x=teams,
        y=probs,
        labels={"x": "Team", "y": "Win Probability (%)"},
        title="2026 F1 Championship Win Probability (ML Model)",
        text=[f"{round(p,1)}%" for p in probs]
    )
    ...
    return html.Div(className="container", children=[
        html.H2("Predictions — ML Based (2026)", className="section-title"),
        dcc.Graph(figure=fig),
        html.Div(className="card", children=[
            html.H4("Machine Learning Model"),
            html.P("Linear Regression trained on historical team performance data from CSV."),
            html.P("Output represents probability (%) of winning the 2026 championship.")
        ])
    ])
...
def page_senna():
    ...
    return html.Div(className="container", children=[
        html.Div(className="card", children=[html.H2("Ayrton Senna — Tribute"), html.P("Ayrton Senna remains one of the greatest Formula 1 drivers of all time. This tribute highlights career, achievements and legacy.")]),
        html.Div(style={"display":"flex","gap":"16px","marginTop":"12px"}, children=[
            html.Img(src="/assets/senna.png", style={"width":"320px","borderRadius":"8px"}),
            html.Div(children=[
        ...
        html.Div(className="card", children=[html.P('"Being second is to be the first of the ones who lose." — Ayrton Senna')]),
        html.Div(className="card", style={"marginTop":"12px"}, children=[html.H4("Senna media / screenshot (local upload)"), html.Img(src="/assets/senna.png", style={"width":"100%","borderRadius":"8px"})])
    ])

# -------------------------
# Layout + router
# -------------------------
app.layout = html.Div(children=[dcc.Location(id="url"), make_nav(), html.Div(id="page-content")])

@app.callback(Output("page-content","children"), Input("url","pathname"))
def router(pathname):
    if pathname == "/teams": return page_teams()
    if pathname and pathname.startswith("/team/"): team_id = pathname.split("/team/")[1]; return page_team_detail(team_id)
    if pathname == "/drivers": return page_drivers()
    if pathname == "/champions": return page_champions()
    if pathname == "/predictions": return page_predictions()
    if pathname == "/senna": return page_senna()
    if pathname == "/about": return page_about()
    return page_home()
